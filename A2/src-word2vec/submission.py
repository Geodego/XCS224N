...
def naive_softmax_loss_and_gradient(center_word_vec, outside_word_idx, outside_vectors, dataset):
    ...
    ### START CODE HERE
    # calculate the predicted proba distribution:
    y_pred = np.exp(outside_vectors @ center_word_vec) / np.sum(np.exp(outside_vectors @ center_word_vec))
    loss = -np.log(y_pred[outside_word_idx])

    # true proba distribution
    voc_size = outside_vectors.shape[0]
    y = np.zeros((voc_size,))
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset,
    ...
    for word in outside_words:
        loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vec,
...
def word2vec_sgd_wrapper(word2vec_model, word2ind, word_vectors, dataset, window_size,
    ...
    for i in range(batchsize):
        ...
        c, gin, gout = word2vec_model(center_word, window_size_1, context, word2ind, center_word_vectors,
