...
class Test_1a(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertEqual(small_ind, small_ind_gold,
        ...
        tgt_sents = [
            ['<s>', "Let's", 'start', 'by', 'thinking', 'about', 'the', 'member', 'countries', 'of', 'the', 'OECD,',
             'or',
            ...
            ['<s>', 'It', 'suggests', 'that', 'we', 'care', 'about', 'the', 'fight,', 'about', 'the', 'challenge.',
             '</s>'],
        ...
        self.assertEqual(tgt_ind, tgt_ind_gold,
                         "target vocab test resulted in indices list {:}, expected {:}".format(tgt_ind,
                                                                                               tgt_ind_gold))
    ...
    def test_1(self):
        ...
        sentences = [
            ['<s>', "Let's", 'start', 'by', 'thinking', 'about', 'the', 'member', 'countries', 'of', 'the', 'OECD,',
             'or', 'the', 'Organization', 'of', 'Economic', 'Cooperation', 'and', 'Development.', '</s>'],
            ['<s>', 'In', 'the', 'case', 'of', 'gun', 'control,', 'we', 'really', 'underestimated', 'our', 'opponents.',
             '</s>'],
            ['<s>', 'Let', 'me', 'share', 'with', 'those', 'of', 'you', 'here', 'in', 'the', 'first', 'row.', '</s>'],
            ['<s>', 'It', 'suggests', 'that', 'we', 'care', 'about', 'the', 'fight,', 'about', 'the', 'challenge.',
             '</s>'], ['<s>', 'A', 'lot', 'of', 'numbers', 'there.', 'A', 'lot', 'of', 'numbers.', '</s>']]
...
class Test_1b(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertEqual(padded_sentences, gold_padded_sentences,
                         "Sentence padding is incorrect: it should be:\n {} but is:\n{}".format(
...
class Test_1f(GradedTestCase):
    def setUp(self):
        ...
        src_sents = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/train.es', source='src')
        tgt_sents = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/train.en', source='tgt')
        self.vocab = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).Vocab.build(
            src_sents, tgt_sents, 50000, 2)

        self.student_embeddings = submission.ModelEmbeddings(H_WORD_EMBEDDING_SIZE, self.vocab.src)
        self.soln_embeddings = self.run_with_solution_if_possible(submission,
                                                                  lambda sub_or_sol: sub_or_sol).ModelEmbeddings(
            H_WORD_EMBEDDING_SIZE, self.vocab.src)
        self.student_embeddings.eval()
        self.soln_embeddings.eval()

        # Read Data
        train_data_src = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/data.es', 'src')
        train_data_tgt = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/data.en', 'tgt')
        train_data = list(zip(train_data_src, train_data_tgt))
        for src_sents, tgt_sents in self.run_with_solution_if_possible(submission,
                                                                       lambda sub_or_sol: sub_or_sol).batch_iter(
                train_data, batch_size=H_BATCH_SIZE, shuffle=False):
            src_sents = src_sents
            tgt_sents = tgt_sents
            break
...
class Test_1g(GradedTestCase):
    def setUp(self):
        # Read Data
        init_seeds_g()
        self.vocab = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).Vocab.load(
            './vocabs/vocab_test.json')
        train_data_src = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/data.es', 'src')
        train_data_tgt = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(
            './en_es_data/data.en', 'tgt')
    ...
    def test_0(self):
        ...
        with torch.no_grad():
            ...
            init_seeds_g()
            soln_nmt = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).NMT(
                G_WORD_EMBEDDING_SIZE, G_HIDDEN_SIZE, self.vocab, no_char_decoder=True)
            soln_nmt.eval()
...
class Test_1h(GradedTestCase):
    ...
    def test_0(self):
        ...
        ref = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(ref_file,
                                                                                                        source='tgt')
        hyp = self.run_with_solution_if_possible(submission, lambda sub_or_sol: sub_or_sol).read_corpus(hyp_file,
                                                                                                        source='tgt')
...
class Test_2d(GradedTestCase):
    ...
    def test_0(self):
        ...
        self.assertEqual(len(decodedWords), BATCH_SIZE,
                         "Length of decodedWords should be {} but is: {}".format(BATCH_SIZE,
                                                                                 len(decodedWords)))
