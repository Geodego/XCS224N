...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = []
    max_len_sent = max(len(s) for s in sents)
    pad_token = [char_pad_token] * max_word_length

    # padd words
    for s in sents:
        s = [w[:max_word_length] for w in s]  # truncates words that are too long
        for i, w in enumerate(s):
            s[i] = w + [char_pad_token] * (max_word_length - len(w))

        # padd sentences now that all the words are padded
        padded = [pad_token] * max_len_sent
        padded[:len(s)] = s
